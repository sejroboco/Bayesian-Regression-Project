...
>>>>>>> 480eb41b1689c2a4eeba582cf7ed70e2849e74d1
\newpage
...
- Permet à chaque variable d'avoir sa propre importance (via $\sigma^2_{\beta_j}$)
- Équivalent au **Ridge bayésien**
...
+ *Adaptabilité* : Les variables importantes peuvent avoir de grandes variances (peu de shrinkage), tandis que les variables non informatives sont fortement pénalisées ;
+ *Réalisme* : Il est peu plausible que toutes les chaînes aient la même influence.
...
- Après `burnIn` itérations, l'algorithme de Gibbs a convergé vers la distribution stationnaire
- Les valeurs suivantes (post-burn-in) sont des échantillons de la loi a posteriori
- L'espérance a posteriori $\mathbb{E}[\theta|Y]$ est estimée par la moyenne empirique des échantillons
...
# Calcul du ratio de shrinkage : |beta_méthode| / |beta_OLS|
...
selection_by_type_detailed <- data.frame(
  Type = unique(extract_channel_type(colnames(X))),
  stringsAsFactors = FALSE
)

# Ajout du nombre total de variables de chaque type
total_by_type <- table(extract_channel_type(colnames(X)))
...
for(method_name in c("RR", "BayesA", "LASSO", "SSVS")) {
  if(method_name == "RR") {
    selected_vars <- colnames(X)[top_15_rr]
  } else if(method_name == "BayesA") {
    selected_vars <- colnames(X)[top_15_bayesA]
  } else if(method_name == "LASSO") {
    selected_vars <- colnames(X)[top_15_lasso]
  } else {
    selected_vars <- colnames(X)[top_15_ssvs]
  }
...
rownames(selection_binary) <- colnames(X)
...
if(length(only_lasso) > 0) {
  ...
  specific_lasso_analysis <- data.frame(
    Variable = colnames(X)[only_lasso],
...
Pour $j = 1, \dots, N$ (nombre d'échantillons souhaités) :
  ...
  Jusqu'à ce que $\rho (S(Y^{*}), S(Y)) \leq \epsilon$ 
