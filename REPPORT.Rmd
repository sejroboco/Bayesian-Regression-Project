...
header-includes:
  ...
  - \fancyfoot[C]{\thepage}
  - \fancyfoot[L]{Ali ABDELWAHID \& Toussaint BOCO}
...
Dans ce travail, on s’intéresse à l’analyse et à la prédiction du score de satisfaction des clients d’une chaîne câblée à partir de leurs usages des différentes chaînes de télévision. Le jeu de données étudié comporte $150$ observations, pour lesquelles un score de satisfaction global est mesuré, ainsi que $160$ variables explicatives correspondant aux temps passés et au nombre de visites sur différentes catégories de chaînes. Les variables explicatives ont été préalablement normalisées.
...
# 0. Importation et exploration des données
...
```{r, results='hide'}
beta_hat_rr <- rr_blup$u
mu_hat_rr <- as.numeric(rr_blup$beta)
```
...
La corrélation entre les scores observés et les scores prédits sur le jeu de test est de `r round(cor_rr, 3)`. Cette valeur indique que le modèle RR-BLUP parvient à capturer une partie importante de la variabilité du score de satisfaction, tout en restant limité par le caractère fortement dimensionné du problème. Cette performance prédictive servira de référence pour la comparaison avec les méthodes bayésiennes plus flexibles étudiées par la suite.

## 1.3. Sélection des variables explicatives

### 1.3.1. Rappels et objectif
...
### 1.3.2. Principe de sélection

Deux approches complémentaires sont possibles :

\begin{itemize}
\item Analyse visuelle à l’aide d’un boxplot des coefficients estimés ;
\item Sélection par seuillage, en retenant les coefficients dont la valeur absolue dépasse un certain quantile élevé de la distribution.
\end{itemize}
...
```{r rr_variable_selection, results='hide'}
...
Le nombre de variables sélectionnées est : `r n_selected_rr` pour un seuil de `r round(seuil_rr, 3)` 
...
**Avantages** :
\begin{itemize}
\item Permet à chaque variable d'avoir sa propre importance (via $\sigma^2_{\beta_j}$)
\item Équivalent au **Ridge bayésien**
\item Estimation via **Gibbs sampler**
\end{itemize}
...
\begin{tabular}{|c|c|c|}
\hline
\textbf{Critère} & \textbf{\textcolor{blue}{Random Regression}} & \textbf{\textcolor{blue}{Bayes A}} \\ \hline
Variance des $\beta_j$ & Commune : $\sigma^2_\beta$ & Spécifique : $\sigma^2_{\beta_j}$ \\ \hline
Flexibilité & Faible : tous les $\beta_j$ régularisés de la même manière & Forte : shrinkage adaptatif\\ \hline
Hyperparamètres & Aucun & $(a, b)$ pour chaque variance \\ \hline
Méthode d'estimation & REML (analytique) & Gibbs sampler (MCMC) \\ \hline
\end{tabular}
...
\begin{itemize}
\item *Adaptabilité* : Les variables importantes peuvent avoir de grandes variances (peu de shrinkage), tandis que les variables non informatives sont fortement pénalisées ;
\item *Réalisme* : Il est peu plausible que toutes les chaînes aient la même influence.
\end{itemize}

## 2.2 Estimation des paramètres

```{r bayesA_estimation, results = "hide"}
...
\begin{itemize}
\item Après `burnIn` itérations, l'algorithme de Gibbs a convergé vers la distribution stationnaire
\item Les valeurs suivantes (post-burn-in) sont des échantillons de la loi a posteriori
\item L'espérance a posteriori $\mathbb{E}[\theta|Y]$ est estimée par la moyenne empirique des échantillons
\end{itemize}
...
\begin{itemize}
\item Minimum : 10-20% du total d'itérations ;
\item Notre choix : `burnIn = 2000` sur `nIter = 12000` $\approx$ 16.7% ;
\item En pratique : observer les traces pour s'assurer de la convergence.
\end{itemize}
...
\begin{itemize}
\item Pas de tendance croissante/décroissante
\item Variance stable tout le long de la chaîne
\item Pas de "plateaux" (blocage de la chaîne)
\end{itemize}
...
\begin{itemize}
\item **Corrélation** : 0.899 vs 0.122 (+678% !)
\item **$R^2$** : 0.808 vs 0.015 (le modèle explique désormais 81% de la variance)
\item **RMSE** : 5.37 vs 10.84 (-50%)
\end{itemize}
...
```{r bayesA_variable_selection, results='hide'}
...
comparison_top <- data.frame(
  Variable = colnames(X)[top_10_bayesA],
...
selected_var_names_bayesA <- colnames(X)[selected_vars_bayesA]
...
En conclusion, le modèle Bayes A offre une flexibilité supérieure au RR-BLUP grâce aux
...
\begin{tabular}{|c|c|c|}
\hline
\textbf{Critère} & \textbf{\textcolor{blue}{Bayes A (Ridge bayésien)}} & \textbf{\textcolor{blue}{Lasso Bayésien}} \\ \hline
A priori sur $\beta$ & Gaussien : $\beta_j \sim \mathcal{N}(0, \sigma^2_{\beta_j})$ & Laplace : $\beta_j \sim \text{Laplace}(0, \lambda)$ \\ \hline
Type de pénalité & L2 : $\sum \beta_j^2$ & L1 : $\sum |\beta_j|$ \\ \hline
Effet de shrinkage & Proportionnel (réduction homogène) & Sélection : force vers zéro exact \\ \hline
Sélection de variables & Non (tous les $\beta_j \neq 0$) & Oui (certains $\beta = 0$ exactement) \\ \hline
Hiérarchie & 1 niveau : $\sigma^2_{\beta_j}$ & 2 niveaux : $\tau_j$ puis $\lambda$ \\ \hline
Densité & Pic moins prononcé en 0 & Pic en 0 (favorise parcimonie) \\ \hline
\end{tabular}
\caption{Différences conceptuelles entre Bayes A et Lasso Bayésien}
\end{table}
...
\hline
\textbf{Valeur de $\lambda$} & \textbf{Effet sur les $\beta$} & \textbf{Variance a priori} & \textbf{Interprétation} \\ \hline
$\lambda$ petit ($\to 0$) & Faible shrinkage & Grande & Proche des moindres carrés \\ \hline
$\lambda$ modéré & Shrinkage équilibré & Modérée & Compromis biais--variance \\ \hline
$\lambda$ grand ($\to \infty$) & Shrinkage fort & Petite & $\beta \to 0$ (sous-ajustement) \\ \hline
\end{tabular}
\caption{Effet du paramètre de régularisation $\lambda$ sur les coefficients}
\end{table}
...
```{r lasso_bayesian_estimation, results='hide'}
...
comparison_dist_df <- data.frame(
  ...
  coefficient = rep(rep(c("beta_important1", "beta_important2", "beta_faible1", "beta_faible2"), 
                        each = n_sim), 2)
)
...
comparison_dist_df$coefficient <- factor(
  ...
  levels = c("beta_important1", "beta_important2", "beta_faible1", "beta_faible2"),
  labels = c(
    "beta[important1]",
    "beta[important2]",
    "beta[faible1]",
...
1. **Coefficients importants** (beta_important1, beta_important2) :
...
2. **Coefficients faibles** (beta_faible1, beta_faible2) :
...
```{r lasso_variable_selection, results='hide'}
# Sélection basée sur |beta| avec seuil adaptatif
...
comparison_selection <- data.frame(
  Comparaison = c("RR beta Vs Bayes A", "RR beta Vs LASSO", "Bayes A beta Vs LASSO", "RR beta Vs Bayes A beta Vs LASSO"),
...
top10_detailed <- data.frame(
  Variable = colnames(X)[top_10_lasso],
...
venn_data <- data.frame(
  Méthode = c("RR-BLUP", "Bayes A", "LASSO", "RR beta Vs Bayes A", "RR beta Vs LASSO", 
              "Bayes A beta Vs LASSO", "Tous"),
  Count = c(
...
- **L1 (LASSO)** : Contrainte en forme de **losange** $\to$ solutions aux **sommets** (axes) $\to$ certains $\beta = 0$
- **L2 (Ridge)** : Contrainte **circulaire** $\to$ solutions rarement sur les
axes $\to$ tous $\beta \neq 0$
...
```{r correlation_effect, results='hide'}
...
if(length(only_bayesA) > 0 & length(only_lasso) > 0) {
  ...
  print(colnames(X)[only_bayesA])
  cat("\nVariables uniquement retenues par LASSO (pas Bayes A):\n")
  print(colnames(X)[only_lasso])
...
\begin{tabular}{|c|c|c|}
\hline
\textbf{Situation} & \textbf{Bayes A (Ridge)} & \textbf{LASSO} \\ \hline
\textbf{Signal fort} & Coefficient modéré (shrinkage proportionnel) & Coefficient préservé (peu de shrinkage) \\ \hline
\textbf{Signal faible} & Coefficient faible mais non nul & Coefficient = 0 (élimination) \\ \hline
\textbf{Signal moyen} & Coefficient réduit & Peut être éliminé ou préservé \\ \hline
\end{tabular}
\caption{Comparaison du comportement de Bayes A et du LASSO selon l’intensité du signal}
\end{table}
...
kable(comparison_methods, digits = 4,
  ...
  column_spec(1, latex_column_spec = "p{7cm}")
```
...
```{r ssvs_estimation, results='hide'}
# Application de SSVS
set.seed(123)
...
top_ssvs_df <- data.frame(
  Variable = colnames(X)[top_10_ssvs],
...
```{r ssvs_trace_example, results='hide'}
...
trace_df <- data.frame(
  ...
  variable = rep(paste0("Var_", trace_indices, " (", 
                        colnames(X)[trace_indices], ")"), each = n_samples),
...
# Configuration 1 : pi faible (a priori parcimonieux)
pi1 <- 0.05
ssvs1 <- selection_SSVS(y_train, X_train, nIter = 80, burnIn = 20,
                        pi_prior = pi1, c_param = 50, r_change = 3)

# Configuration 2 : pi élevé (a priori généreux)
pi2 <- 0.3
...
comparison_hyper <- data.frame(
  Configuration = c(
    ...
    "pi faible (pi=0.05, c=50)",
    "pi élevé (pi=0.3, c=50)",
...
sensitivity_df <- data.frame(
  ...
  Config = rep(c("pi=0.1 (référence)", "pi=0.05 (strict)", "pi=0.3 (généreux)"),
...
ggplot(sensitivity_df, aes(x = Frequency, fill = Config)) +
  ...
  labs(title = "Distribution des fréquences de sélection selon pi",
...
\begin{tabular}{|c|c|c|c|}
\hline
$\pi$ & \textbf{Interprétation} & \textbf{Nb. variables sélectionnées} & \textbf{Usage} \\ \hline
\textbf{Faible} (0.05) & A priori très parcimonieux & Peu (seuil élevé) & Pour un modèle très simple \\ \hline
\textbf{Modéré} (0.1) & Équilibre & Modéré & Cas général \\ \hline
\textbf{Élevé} (0.3) & A priori inclusif & Beaucoup & Exploration, éviter faux négatifs \\ \hline
\end{tabular}
\caption{Influence du paramètre $\pi$ sur la sélection de variables (SSVS)}
\end{table}
...
\begin{tabular}{|c|c|c|c|}
\hline
$c$ & \textbf{Interprétation} & \textbf{Effet} & \textbf{Usage} \\ \hline
\textbf{Faible} (10) & Faible confiance dans les données & Sélection conservatrice & Données bruitées \\ \hline
\textbf{Modéré} (50--100) & Confiance raisonnable & Équilibre & Cas général \\ \hline
\textbf{Élevé} (200+) & Forte confiance dans les données & Sélection agressive & Données fiables, $n$ grand \\ \hline
\end{tabular}
\caption{Influence du paramètre $c$ sur la sélection de variables}
\end{table}
...
\hline
\textbf{Méthode} & \textbf{Type de pénalité} & \textbf{Sélection exacte ?} & \textbf{Philosophie} \\ \hline
RR-BLUP & Aucune (variance commune) & Non & Régression ridge la plus simple \\ \hline
Bayes A & L2 (quadratique) & Non & Ridge bayésien avec variances individuelles \\ \hline
LASSO Bayésien & L1 (absolue) & Oui (soft) & Parcimonie via shrinkage fort \\ \hline
\end{tabular}
\end{table}
...
\begin{tabular}{|c|c|c|c|}
\hline
\textbf{Méthode} & \textbf{Approche} & \textbf{Sélection exacte ?} & \textbf{Philosophie} \\ \hline
SSVS & Recherche stochastique & Oui (hard) & Exploration de l'espace des modèles \\ \hline
\end{tabular}
\end{table}
...
Il est important de souligner qu'un shrinkage fort n'est pas toujours meilleur !
...
kable(performance_df %>% select(Méthode, Corrélation, Écart_Cor_pct, RMSE, Écart_RMSE_pct),
      digits = 2,
      col.names = c("Méthode", "Corrélation", "Écart relatif (%)", "RMSE", "Écart (%)"),
...
**Recommandation** :
  ...
  - **Pour l'interprétation** : préférer LSSO (parcimonie) ou SSVS (probabilités)
...
selected_all_methods <- lapply(list(
  RR = colnames(X)[top_15_rr],
  BayesA = colnames(X)[top_15_bayesA],
  LASSO = colnames(X)[top_15_lasso],
  SSVS = colnames(X)[top_15_ssvs]
), extract_channel_type)

# Comptage par type

# Récupération de tous les types possibles
all_types <- names(table(extract_channel_type(colnames(X))))
...
comparison_detailed <- data.frame(
  Rang = 1:10,
  RR_BLUP = colnames(X)[top_10_rr[1:10]],
  Bayes_A = colnames(X)[top_10_bayesA[1:10]],
  LASSO = colnames(X)[top_10_lasso[1:10]],
  SSVS = colnames(X)[top_10_ssvs[1:10]]
)
...
if(nrow(consensus_4) > 0) {
  consensus_4_indices <- match(consensus_4$Variable, colnames(X))
...
consensus_indices <- match(top_consensus$Variable, colnames(X))
...
- Les approches bayésiennes sont en moyenne`r ifelse(mean(all_methods_comp$Corrélation[1:3], na.rm=TRUE) > mean(all_methods_comp$Corrélation[5:7]), "légèrement supérieures", "comparables")` aux fréquentistes
...
## 5.7 Comparaison des variables sélectionnées
...
if(length(ultra_consensus) > 0) {
  cat("\nCes variables sont :\n")
  print(colnames(X)[ultra_consensus])
}
```
...
final_vars_names <- colnames(X)[final_vars_indices]
...
```{r final_model_summary, results='hide'}
# Résumé du modèle
...
knitr::kable(
  coef_table,
  digits = 4,
  caption = "Résumé du modèle final (coefficients)",
  escape = TRUE
)
```
...
```{r rebuild_coef_df, results="hide"}
# Reconstruction de coef_df pour les analyses suivantes
...
# Renommer l'intercept pour éviter les problèmes LaTeX
...
```{r significant_vars, results='hide'}
...
```{r coefficient_interpretation, results='hide'}
...
```{r residuals_final, results='hide'}
...
kable(final_comparison, digits = 4,
  ...
  kable_styling(latex_options = "hold_position") %>%
  row_spec(nrow(final_comparison), bold = TRUE, background = "blue")
```
...
+ La vraisemblance $f\left(Y|\theta\right)$ est difficile ou impossible à calculer analytiquement ;
...
Pour $j = 1, \dots, N$ (nombre d'échantillons souhaités) :
\begin{itemize}
\item Répéter :
  \begin{itemize}
    \item 1. Générer $\theta^{*} \sim \pi (\theta)$
    \item 2. Simuler $Y^{*} \sim f(\cdot \mid \theta^{*})$
    \item 3. Calculer distance $\rho (S(Y^{*}), S(Y))$
  \end{itemize}
\item Jusqu'à ce que $\rho (S(Y^{*}), S(Y)) \leq \epsilon$ 
\item Retenir $\theta^{*}(j) = \theta^{*}$
\end{itemize}
Fin
...
**Limite** : Quand $\varepsilon \to 0$ et si $S(Y)$ est une statistique **suffisante**, alors $\pi_{ABC}(\theta) \to \pi(\theta | Y)$.

### Application au modèle de régression

Pour notre modèle :

$$Y = \mu \mathbb{I} + X\beta + \epsilon, \quad \epsilon \sim \mathcal{N}(0, \sigma^2_\epsilon I)$$

- **Paramètres** : $\theta = (\mu, \beta, \sigma^2_\epsilon)$
- **Vraisemblance** : $f(Y|\theta) = (2\pi\sigma^2_\epsilon)^{-n/2} \exp\left(-\frac{1}{2\sigma^2_\epsilon}\|Y - \mu\mathbf{I} - X\beta\|^2\right)$
...
**Critère d'acceptation** : On accepte $\theta$* si **p-value > seuil** (distributions non significativement différentes)
...
# Graphique comparatif pour sigma^2
...
Le taux d’acceptation obtenu avec la méthode ABC standard est d’environ `r round(100*abc_result$acceptance_rate, 2)` %, tandis que celui obtenu avec l’approche ABC fondée sur le test de Kolmogorov–Smirnov (KS) est d’environ `r round(100*abc_ks_result$acceptance_rate, 2)` %.  
Le test KS étant généralement plus strict, il conduit logiquement à un taux d’acceptation plus faible.

**2. Précision des estimations**

Les deux méthodes produisent des estimations qui convergent vers des valeurs similaires. Toutefois, l’approche ABC-KS peut introduire un léger biais lorsque le test est paramétré de manière trop conservatrice, ce qui augmente le risque de rejeter des simulations pourtant compatibles.

**3. Interprétation**

L’ABC standard repose sur un seuil ($\epsilon$), choisi de manière arbitraire, qui contrôle directement la tolérance entre données simulées et observées.  
À l’inverse, l’ABC basé sur le test KS utilise une p-value, offrant une interprétation statistique plus explicite du critère d’acceptation.

**4. Avantages du test KS**

- Pas besoin de définir des statistiques résumées  
- Critère d’acceptation fondé sur une p-value, donc directement interprétable  
- Exploite l’ensemble de la distribution empirique

**5. Limites du test KS**

- Peut être trop conservateur et rejeter un nombre excessif de simulations  
...
### Impact de $\varepsilon$ sur ABC standard
...
4. **Approximation (pas exact)**
   ...
   - Qualité dépend de $\varepsilon$ et des statistiques
...
1. Accepter avec $\varepsilon$ large (beaucoup d'échantillons)
