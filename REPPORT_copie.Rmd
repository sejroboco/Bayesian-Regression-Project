...
\begin{itemize}
\item \textbf{Adaptabilité} : Les variables importantes peuvent avoir de grandes variances (peu de shrinkage), tandis que les variables non informatives sont fortement pénalisées ;
\item \textbf{Réalisme} : Il est peu plausible que toutes les chaînes aient la même influence.
\end{itemize}
...
\begin{itemize}
\item \textbf{Corrélation} : 0.899 vs 0.122 (+678% !)
\item \textbf{$R^2$} : 0.808 vs 0.015 (le modèle explique désormais 81% de la variance)
\item \textbf{RMSE} : 5.37 vs 10.84 (-50%)
\end{itemize}
